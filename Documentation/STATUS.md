...
### UI-4C — SIMPLIFIED SEARCH VOLUME TO VALIDATION UX

- Safely refactored the Search Volume component locally without an external formatter; the temporary Stage A backup was removed after validation.
- Preserved Search Volume evidence, qualification, sorting, trends, Commercial Insights, copy/export, and handoff API behavior.
- Made handoff and result-action controls explicit `type="button"` controls so they cannot submit the research form accidentally.
- A successful single-handoff response now uses the exact returned `all_handoff_ids` value for automatic navigation to `/rank-rent/validator?handoff_id=...`; duplicate/reused handoffs use the returned existing ID, while multiple or zero IDs use the generic validator route.
- Validator now resolves a saved local project when available, creates one only when no project is available, automatically attaches imported handoffs, refreshes the existing zero-network preview, and enables Start Validation only after backend-confirmed executable candidates are attached.
- Error categories remain visible as API ERROR, VALIDATION REJECTED, or RUN FAILED; provider infrastructure, authentication/quota infrastructure, and Search Volume provider semantics remain frozen.
- Validation: `npm.cmd exec tsc -- --noEmit --incremental false` passed; focused UI-4C backend tests passed (3); `git diff --check` passed. No external provider requests were made.
- Next product work after UI-4C remains UI-4D Candidate Table + Stage Timeline. No commit or push was made.

### UI-4C browser acceptance diagnosis and repair

- Browser review exposed a real initialization defect: country-targeted Search Volume handoffs had no city/state in `location_target`, while the existing attachment endpoint correctly requires a city-targeted candidate.
- The frontend therefore remained in an internal-looking setup state after the attachment request returned `HANDOFF_CITY_UNRESOLVED`.
- The existing attachment path now performs a zero-network, local-registry-only resolution when exactly one city embedded in the localized keyword matches; ambiguous or absent cities still fail explicitly.
- Validator now presents the imported keyword, stored SV, and evidence reuse prominently, and reports initialization failures as `INITIALIZATION ERROR` instead of leaving a misleading setup state.
- Frozen provider, evidence, policy, authentication, quota, and run architecture was preserved. No new candidate model or provider request was introduced.
- Validation after repair: focused UI-4C backend tests 3 passed, frontend TypeScript passed, and `git diff --check` passed. Manual browser rerun is still required before UI-4C acceptance/freeze.
- Follow-up diagnosis found initialization could also appear indefinitely busy because project/attachment/preview were split across effects and fetches had no timeout-safe state machine. Initialization is now one guarded sequential flow with `idle`, `setting_up_project`, `attaching_candidate`, `previewing`, `ready`, and `initialization_error` states; the returned project ID is used directly for attachment and preview.
- Invalid saved projects are cleared and retried once; completed failures leave loading states and show `INITIALIZATION ERROR`. A regression test covers a country-targeted localized handoff becoming a `ProjectCandidate` while preserving the exact evidence ID.
- Validation after the state-machine repair: focused UI-4C backend tests 4 passed, frontend TypeScript passed, and `git diff --check` passed. Manual browser rerun remains required; UI-4C is not yet marked accepted.

### UI-4C runtime route diagnosis

### Alembic c15 ancestry correction and active database reconciliation

- Corrected only the uncommitted `c15_project_candidate_sv_evidence.py` ancestry: `down_revision` changed from the malformed `("c13userquotas", "f1trialmode")` merge declaration to `"c13userquotas"`; `f1trialmode` is already an ancestor of `c13userquotas`.
- Preserved the existing `nicheforge.pre_c11_reconciliation.20260816.sqlite` backup and created `nicheforge.pre_c15_graph_repair.20260816.sqlite`, verified byte-for-byte against the pre-upgrade active database.
- Existing-database copy validation passed: version metadata was reconciled to `c13userquotas`, then normal `alembic upgrade head` applied only `c15` successfully.
- Fresh-database validation passed from base through `c15projectcandidatesvevidence`.
- Active database metadata was reconciled with `alembic stamp --purge c13userquotas`; normal `alembic upgrade head` then applied `c15`.
- Final Alembic state: current and sole head are `c15projectcandidatesvevidence`. `project_candidates.search_volume_evidence_id`, its index, and foreign key are present.
- Preserved row counts: `fx_rate_evidence=1`, `provider_calls=24`, `keyword_metric_validation_handoffs=14`, `project_candidates=0`, `users=1`, `user_sessions=0`, `user_provider_quotas=0`, `run_reservations=0`.
- Backend restarted from the project `.venv`; `/health/live` and `/api/v1/rank-rent/handoffs` both return 200. Provider/network requests remained zero. No commit or push was made.

- Browser logs identified `GET /api/v1/rank-rent/handoffs` returning 404 while other API routes remained available.
- Source inspection confirms the handlers are in `backend/app/api/routes.py` under router prefix `/api/v1`, and `backend/app/main.py` includes that router.
- The imported FastAPI app and `TestClient(app)` expose the canonical collection route and return 200; unknown handoffs return 404 as expected.
- The currently running Uvicorn process has a stale/different route table: its live OpenAPI omits `rank-rent/handoffs` and its live request returns 404. No application router change was required; restart the backend from the current project code before browser acceptance.
- Added a route-registration regression test. Focused UI-4C tests: 5 passed; no provider requests; `git diff --check` passed.
...
# Local city registry — UI-4C follow-up (2026-08-16)

- Populated the existing `cities` table from the bundled local Census place-estimate source `backend/data/census/SUB-EST2025.csv`.
- Import was local-only and used the existing `import_cities` service; no parallel lookup table or provider request was added.
- Rows: 0 before, 19,465 after; 19,465 accepted, 0 rejected, 18 source conflicts safely skipped.
- Albany now has genuine local matches including Albany, GA; Albany, NY; and Albany, OR, enabling the Validator ambiguity selector.
- The import is rerunnable through the existing file-hash/idempotent import path.
- Required statement: Local US city registry populated for zero-network Rank & Rent city resolution.
# Active DB RunCandidate schema reconciliation — UI-4C follow-up (2026-08-16)

- Root cause: historical migration `d2a1f0c4e7b9_add_adaptive_authority_fields.py` was marked in the Alembic ancestry but only partially applied physically.
- Missing physical columns were `run_candidates.adaptive_seek_ideal_used` and `runs.adaptive_seek_ideal`; all other audited ORM columns/indexes/foreign keys were present.
- Verified copy first using `nicheforge.pre_runcandidate_schema_repair.20260816.sqlite`, then applied the same narrow reconciliation to `nicheforge.db`.
- Alembic revision metadata was not changed. No provider calls were made.
# UI-4C final result truthfulness hardening (2026-08-16)

- Canonical Search Volume handoff evidence is `keyword_metric_evidence`; preview now resolves the linked `ProjectCandidate.search_volume_evidence_id` from that store, preserving immutable SV/provider lineage.
- Preview exposes the reusable value (for example, `390 · Reused`) instead of only a stage count.
- Run responses now separate lifecycle progress from candidate outcome and expose stage/result details. A completed retryable SERP run reports 100% lifecycle progress, `RETRYABLE`, `SERP_INSUFFICIENT_ORGANIC_RESULTS`, downstream stages not run, and final result not produced.
- Existing `(run_id, project_candidate_id)` uniqueness continues to guarantee one RunCandidate per ProjectCandidate per run; repeated handoffs reuse the existing project candidate.
- Focused tests: 12 passed. Frontend TypeScript passed with `npx tsc --noEmit --incremental false`. Provider/network calls during tests: 0.
# UI-4C SERP diagnostic and stage explanations (2026-08-16)

- Stored SERP evidence for run `2fa5384e-fb04-4238-a4db-22eaf3e8a166` used keyword `tree services albany`, localized to Albany, NY, with requested depth 10.
- DataForSEO returned zero usable organic results because the stored response rejected the request with status `40501 Invalid Field: 'location_name'`.
- Classification: F — provider request/response schema mismatch. This was not a genuine SERP shortage, wrong-city resolution, or Rank & Rent rejection.
- No new DataForSEO request was made.
- Corrected the DataForSEO SERP factory wiring so live/trial SERP requests resolve the exact city/state to a `location_code`; the invalid `location_name` fallback is no longer used when the resolver is available.
- Added `SERP_PROVIDER_REQUEST_ERROR` classification so rejected provider requests are not recorded as genuine `SERP_INSUFFICIENT_ORGANIC_RESULTS`.
- Validator run results now include concise explanations for reused SV, retryable SERP, dependent DA/KD stages, and an unproduced final result.
# UI-4C result presentation hardening implementation (2026-08-16)

- Run responses now expose structured per-candidate stage results instead of a concatenated paragraph.
- Candidate rows are independently expandable for single- and multi-candidate runs.
- Captured SERP evidence and authority evidence are exposed when present, including position, domain, URL, DA, PA, and provider where persisted.
- Partial evidence remains visible; missing downstream stages remain explicitly `NOT RUN` and final decisions remain `NOT PRODUCED` when appropriate.
- Deep Analysis UI uses only currently persisted authority/SERP evidence. No unsupported backlink, spam-score, page-classification, or local-business metrics were invented.
- Automated validation passed: 16 focused backend tests, TypeScript, and `git diff --check`.
- Browser acceptance for one and multiple candidates is still required before UI-4C can be frozen.
