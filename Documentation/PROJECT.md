...
The canonical visual/product roadmap is maintained in
`NicheForge UI UX Redesign Roadmap.md`. It covers the completed app shell and
Search Volume workspace, authenticated allowance feedback, and the next Rank &
Rent Engine/UI phases. UI work must preserve evidence/policy separation,
truthful unavailable states, provider-neutral targeting, and the staged
workflow defined by this project.

The Search Volume → Rank & Rent handoff additions in that roadmap are also
canonical: the handoff threshold is configurable and initially 260, qualifying
rows are only candidates for deeper validation, handoff reuses stored evidence,
and no provider work occurs until the user starts Rank & Rent validation.
