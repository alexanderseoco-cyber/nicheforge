...
## Rank & Rent and General Niche validation pipelines — approved workflow

NicheForge supports two explicit validation scopes. Classification occurs once
at the Search Volume handoff/materialization boundary and is preserved on each
project candidate and run candidate.

`LOCAL_RANK_RENT`: Population → Search Volume → localized SERP → Authority/DA → Deep Analysis → KD → Result.

`GENERAL_NICHE`: Search Volume → national SERP → Authority Opportunity → Deep Analysis → KD → Result.

General candidates preserve their country target, do not enter city resolution,
and show population as `NOT APPLICABLE`. General authority is advisory and
graded: 4+ weak domains = `STRONG_POTENTIAL`, 3 = `GOOD_POTENTIAL`, 1–2 =
`POTENTIAL_NICHE`, and 0 = `LOW_AUTHORITY_OPPORTUNITY`. One or two weak domains
continue to Deep Analysis and KD; zero may stop only when explicitly configured
as a hard gate. Results preserve `authority_opportunity_reason`, weak count,
threshold, coverage, and captured evidence. Moz DA is never labeled Ahrefs DR.

Mixed handoffs are independent per keyword: a local candidate waiting for city
confirmation must not block general candidates. Existing local behavior,
evidence lineage, and zero-network preview guarantees remain intact.
