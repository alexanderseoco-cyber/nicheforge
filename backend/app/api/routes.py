...
def create_run(project_id: str, payload: RunCreate, db: Session = Depends(get_db)):
    ...
    run = Run(project_id=project_id, min_population=profile.min_population, max_population=profile.max_population,
              ...
              minimum_organic_rows=profile.minimum_organic_rows, minimum_organic_coverage=profile.minimum_organic_coverage,
...
def _run_response(db: Session, run: Run):
    ...
    for row in rows:
        ...
        # Preserve compact SERP provenance for the frontend without exposing raw payloads.
        serp_raw = snap.raw_payload if snap and isinstance(snap.raw_payload, dict) else {}
        serp_response = serp_raw.get("response") if isinstance(serp_raw.get("response"), dict) else {}
        serp_status_code = serp_response.get("status_code")
        ...
        serp_coverage = classify_serp_coverage(requested_depth=run.organic_depth, usable_organic_count=len(serp_rows), minimum_organic_rows=policy_rows, minimum_organic_coverage=policy_coverage, provider_success=serp_status_code in (None, 20000)) if snap else None
        serp_valid = bool(serp_coverage and serp_coverage.sufficient_for_downstream)
        serp_state = "PROVIDER ERROR" if serp_status_code not in (None, 20000) else ("VALID" if serp_valid else ("PARTIAL" if snap else "NOT AVAILABLE"))
    ...
    for result in results:
        rc_for_result = db.get(RunCandidate, result["run_candidate_id"])
        snap_for_result = db.get(SerpSnapshot, rc_for_result.serp_snapshot_id) if rc_for_result and rc_for_result.serp_snapshot_id else None
        raw_for_result = snap_for_result.raw_payload if snap_for_result and isinstance(snap_for_result.raw_payload, dict) else {}
        response_for_result = raw_for_result.get("response") if isinstance(raw_for_result.get("response"), dict) else {}
        policy_rows, policy_coverage = resolve_serp_policy(requested_depth=run.organic_depth, minimum_organic_rows=run.minimum_organic_rows, minimum_organic_coverage=run.minimum_organic_coverage)
        coverage = classify_serp_coverage(requested_depth=run.organic_depth, usable_organic_count=result["serp_count"], minimum_organic_rows=policy_rows, minimum_organic_coverage=policy_coverage, provider_success=response_for_result.get("status_code") in (None, 20000)) if snap_for_result else None
        result.update({"serp_provider": snap_for_result.provider if snap_for_result else None, "serp_target": snap_for_result.location_name if snap_for_result else None, "serp_fetched_at": snap_for_result.fetched_at.isoformat() if snap_for_result and snap_for_result.fetched_at else None, "serp_snapshot_id": snap_for_result.id if snap_for_result else None, "serp_evidence_state": coverage.evidence_state if coverage else "NOT AVAILABLE", "serp_requested_depth": coverage.requested_depth if coverage else run.organic_depth, "serp_observed_depth": coverage.usable_organic_count if coverage else 0, "serp_coverage_ratio": coverage.coverage_ratio if coverage else 0.0, "serp_provider_status_code": response_for_result.get("status_code"), "serp_provider_status_message": response_for_result.get("status_message")})
