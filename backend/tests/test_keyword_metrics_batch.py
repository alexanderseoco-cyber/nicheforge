...
async def test_fresh_cache_hits_never_call_provider():
    ...
    result = await KeywordMetricsBatchOrchestrator(provider, {("term", "City", "en"): cached}).execute([request])
...
async def test_only_stale_and_missing_items_are_batched():
    ...
    cache = {("fresh", "City", "en"): CachedMetric(KeywordMetricResult("fresh", 1), True), ("stale", "City", "en"): CachedMetric(KeywordMetricResult("stale", 1), False)}
